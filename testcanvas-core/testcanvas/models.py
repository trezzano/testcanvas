...
def generate_compact_node_uid() -> str:
    ...
    Note:
        Kept for backward compatibility because historical ``0001`` migrations
        reference it. New records use the prefixed, human-friendlier generators
...
# collisions negligible even across large data sets, so the value stays safe as
...
def generate_prefixed_uid(prefix: str) -> str:
    ...
    semantic prefix makes the reference self-descriptive (an LLM can tell an
    Acceptance Criterion UID from a Flow Node UID at a glance) while the token
    keeps it collision-resistant and URL-safe.

    Args:
        prefix: Short semantic prefix identifying the owning model
            (e.g. ``"ACU"`` for an Acceptance Criterion UID).

    Returns:
        A string like ``"ACU-4k9Fm2Xz8Qw1B"``.
    """
    token = "".join(
        secrets.choice(_BASE62_ALPHABET)
