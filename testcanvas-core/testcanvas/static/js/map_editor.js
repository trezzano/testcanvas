...
(function () {
    ...
    // The Operations button and its reopen tab control only the fixed right sidebar.
    ...
    /* ---- Left sidebar resizing ----
       Mirrors the traceability detail-sidebar behavior: drag the inner edge,
       disable the width transition while dragging and clamp to a sensible range.
       The width is written to the `--sb-width` CSS variable (NOT an inline
       `width`) so `.map-sidebar.collapsed { width: 0 }` still wins and the
       collapse/expand button keeps working after a manual resize. */
    function initSidebarResize(options) {
        const sidebarEl = options.sidebar;
        ...
        const onResizeMove = (event) => {
            ...
            sidebarEl.style.setProperty('--sb-width', newWidth + 'px');
            cy.resize();
        };
        ...
        function startResize(pointerX) {
            ...
            // Keep the resize cursor while dragging over the graph/iframe area.
    ...
    initSidebarResize({
        sidebar: sidebar,
        handle: document.getElementById('map-sidebar-resize'),
    });
